...
We previously created groupings based on days of the week.  Another way to implement this is by using the `weekdays()` function.
```{r}
test <- activity.impute
test$date <- as.Date(activity.impute1$date, format = "%Y-%m-%d")   
    # changes the $date column from character class to date class
test$day <- weekdays(activity.impute1$date, abbreviate = FALSE)
```
